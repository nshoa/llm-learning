...
def generate_with_single_input(
    ...
    **kwargs,
    ...
    if (not together_api_key) and ("TOGETHER_API_KEY" not in os.environ):
        url = os.path.join(
            os.environ.get(
                "TOGETHER_BASE_URL", "https://proxy.dlai.link/coursera_proxy/together"
            ),
            "v1/chat/completions",
        )
...
def display_widget(llm_call_func):
    ...
    label1 = widgets.Label(
        value="With RAG", layout={"width": "45%", "text_align": "center"}
    )
    label2 = widgets.Label(
        value="Without RAG", layout={"width": "45%", "text_align": "center"}
    )
    ...
    hbox_labels = widgets.HBox(
        [label1, label2], layout={"justify_content": "space-between"}
    )
